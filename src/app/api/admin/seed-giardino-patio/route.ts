...
const TIPOLOGIE_PREFIXES = ["GIARDINO_PONZA", "CORFU_", "GIARDINO94_"];
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Optional (comuni e specifici per prodotto, scoping via gruppo "GIARDINO E PATIO")
    const optionaliNuovi = optionaliData as OptionalRow[];
    const optionaliEsistenti = await prisma.optional.findMany({
      where: { brandId: brand.id, gruppiApplicabili: { has: "GIARDINO E PATIO" } },
    });
    ...
    for (const o of optionaliNuovi) {
      ...
      if (!esistente) {
        ...
        await prisma.optional.update({
          where: { id: esistente.id },
          data: { valore: o.valore, tipoPrezzo: o.tipoPrezzo, gruppiApplicabili: o.gruppiApplicabili },
        });
        optAggiornati++;
      } else {
        optInvariati++;
