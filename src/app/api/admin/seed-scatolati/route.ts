...
import prodottiData from "../../../../../prisma/seed-data/scatolati_prodotti.json";
import modelliData from "../../../../../prisma/seed-data/scatolati_modelli.json";
import optionaliData from "../../../../../prisma/seed-data/scatolati_optional.json";

export const dynamic = "force-dynamic";

const SECRET = process.env.SEED_SECRET || "gpc-2026-seed-x7f2";
const BRAND = "P&C";
// Scatolati (accessorio per zanzariere, ma famiglia prodotto a se' stante, gruppo
// "SCATOLATI_PC") — un "listino" per formato (60x30/50x20), per scoperire l'unico
// optional (Tappo) che e' specifico del formato ma vale per tutte le finiture.
const LISTINI_SCATOLATI = ["SCATOLATO_60X30", "SCATOLATO_50X20"];
...
const TIPOLOGIE_PREFIXES = ["SCATOLATO_"];
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const optionaliEsistenti = await prisma.optional.findMany({
      where: { brandId: brand.id, listino: { in: LISTINI_SCATOLATI } },
    });
