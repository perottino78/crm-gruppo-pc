...
export default async function SchedaCommercialePage({
  ...
  return (
    <div className="max-w-5xl">
      <Link href="/commerciali" className="text-xs text-neutral-400 hover:underline">
        ← Team &amp; performance
      </Link>
      <div className="flex items-center justify-between mt-2 mb-1">
        <h1 className="text-xl font-medium">{utente.nome}</h1>
        <span className="text-xs px-2 py-1 rounded-full bg-neutral-100 text-neutral-600">{utente.ruolo}</span>
      </div>
      <p className="text-sm text-neutral-400 mb-6">{utente.email} · in squadra dal {utente.createdAt.toLocaleDateString("it-IT")}</p>

      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-lg border border-neutral-200 p-4">
          <p className="text-xs text-neutral-400 mb-1">Preventivi aperti</p>
          <p className="text-lg font-semibold">{preventiviAperti.length}</p>
          <p className="text-xs text-neutral-400">{valoreAperti.toLocaleString("it-IT", { style: "currency", currency: "EUR" })}</p>
        </div>
        <div className="bg-green-50 border border-green-100 rounded-lg p-4">
          <p className="text-xs text-green-600 mb-1">Contratti chiusi (accettati)</p>
          <p className="text-lg font-semibold text-green-800">{preventiviAccettati.length}</p>
          <p className="text-xs text-green-600">{valoreAccettati.toLocaleString("it-IT", { style: "currency", currency: "EUR" })}</p>
        </div>
        <div className="bg-white rounded-lg border border-neutral-200 p-4">
          <p className="text-xs text-neutral-400 mb-1">Tasso di chiusura</p>
          <p className="text-lg font-semibold">{tassoChiusura}%</p>
          <p className="text-xs text-neutral-400">{preventiviPersi.length} persi (scaduti/annullati)</p>
        </div>
        <div className="bg-neutral-900 rounded-lg p-4">
          <p className="text-xs text-neutral-300 mb-1">Appuntamenti eseguiti</p>
          <p className="text-lg font-semibold text-white">{appuntamentiEseguiti.length}</p>
      ...
      <div className="bg-white rounded-lg border border-neutral-200 p-4 mb-6">
        <h2 className="text-sm font-medium text-neutral-700 mb-3">Obiettivo del mese — {labelPeriodo(periodo)}</h2>
        {obiettivoCorrente ? (
          <div className="mb-3">
            <div className="flex items-center justify-between text-sm mb-1">
              <span>{realizzatoMese.toLocaleString("it-IT", { style: "currency", currency: "EUR" })} realizzato</span>
              <span className="text-neutral-400">obiettivo {obiettivoCorrente.importoTarget.toLocaleString("it-IT", { style: "currency", currency: "EUR" })}</span>
            </div>
            ...
            <p className="text-xs text-neutral-400 mt-1">{percentualeObiettivo}% dell&apos;obiettivo raggiunto</p>
          </div>
        ) : (
          <p className="text-xs text-neutral-400 mb-3">Nessun obiettivo impostato per questo mese.</p>
        )}
      ...
      <div className="bg-white rounded-lg border border-neutral-200 p-4 mb-6">
        <h2 className="text-sm font-medium text-neutral-700 mb-1">Permessi listino</h2>
        <p className="text-xs text-neutral-400 mb-3">Aziende/famiglie per cui questo utente è autorizzato a operare (impostati dall&apos;amministratore). Nessuna spunta = accesso a tutti i listini.</p>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white rounded-lg border border-neutral-200 p-4">
          <h2 className="text-sm font-medium text-neutral-700 mb-3">Preventivi ({utente.preventivi.length})</h2>
          <div className="flex flex-col divide-y divide-neutral-50 max-h-80 overflow-y-auto">
            {utente.preventivi.map((p) => (
              ...
              >
                <span>{p.cliente.nome} <span className="text-xs text-neutral-400">· {p.brand.nome}</span></span>
                <span className="flex items-center gap-2">
                  <span className="text-xs text-neutral-500">{p.totaleNetto.toLocaleString("it-IT", { style: "currency", currency: "EUR" })}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-500">{p.stato}</span>
                </span>
              </Link>
            ))}
            {utente.preventivi.length === 0 && <p className="text-xs text-neutral-300 italic py-2">Nessuno</p>}
          </div>
        </div>
        <div className="bg-white rounded-lg border border-neutral-200 p-4">
          <h2 className="text-sm font-medium text-neutral-700 mb-3">Appuntamenti ({utente.appuntamenti.length})</h2>
          <div className="flex flex-col divide-y divide-neutral-50 max-h-80 overflow-y-auto">
            {utente.appuntamenti.map((a) => (
              <div key={a.id} className="text-sm py-1.5">
                <div className="flex items-center justify-between">
                  <span>{a.cliente.nome} <span className="text-xs text-neutral-400">· {a.tipo}</span></span>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-500">{a.stato}</span>
                </div>
                <p className="text-xs text-neutral-400">{a.dataOra.toLocaleString("it-IT")}</p>
              </div>
            ))}
            {utente.appuntamenti.length === 0 && <p className="text-xs text-neutral-300 italic py-2">Nessuno</p>}
          </div>
        </div>
      </div>
    </div>
